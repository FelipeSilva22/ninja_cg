...
if (estadoAtual === "formacao") {
    ...
    if (type === "Chakra" && cardDestino) {
        ...
        aplicarEfeito(cardOrigem, cardDestino, cardOrigem.dataset.effect, 100, "J1");
...
function validarEvolucao(cardOrigem, cardDestino, slotOrigem, slotDestino) {
    ...
    if (
      ...
      console.log(`Bloqueio (${bloqueioEvo}), reg. origem (${regNOrigem}) e reg. destino (${regNDestino})`);
      alert("Evolução inválida: Verifique o estágio, regN ou bloqueio de evolução.");
      return;
    } 
}  
function evoluirNinja(ninjaAntigo, ninjaNovo) { 
    console.log(`🔼 Evoluindo Ninja: ${ninjaAntigo.dataset.name} → ${ninjaNovo.dataset.name}`);
    ...
    ninjaNovo.dataset.hp = (parseInt(ninjaNovo.dataset.hpInicial, 10) || 0) - danoSofrido;
    ninjaNovo.dataset.chakra = (parseInt(ninjaNovo.dataset.chakraInicial, 10) || 0) - chakraGasto;
